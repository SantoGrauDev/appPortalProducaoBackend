...
$array = $read->fetchAll();   
    ...
    foreach ($array as $dados): 
                   ...
                   $VENDAS = $dados["VENDAS"];  
                   $ENCAMINHAMENTO = 2 * 10 * $diasMesEfe;
                   $CONVERSAO =  $VENDAS * 100 / $ENCAMINHAMENTO;   
                   ...
                   $VendaVideoPatricia["VendaVideoPatricia"][] = [
...
$read = $conn->prepare("SELECT DISTINCT  SUM(a.LCTVALOR) AS VENDAVIDEO,sum(a.LCTVALOR)/COUNT(e.PEDSEQUENCIAL) AS TM, k.NOME AS VENDEDOR,n.TVNDESCRICAO,m.MTVVALORMETA AS META
    FROM TB_LCT_LANCAMENTOS a
    INNER JOIN TB_LTV_LANCAMENTOVENDA b ON b.LCTID = a.LCTID 
    INNER JOIN TB_VEN_VENDA c ON c.VENID = b.VENID 
    INNER JOIN TB_VPE_VENDAPEDIDOS d ON d.VENID_VENDA = c.VENID 
...
foreach ($array as $dados): 
        ...
        $VendaVideoPamela["VendaVideoPamela"][] = [
...
//*******************************************************************FIM DO SEGUNDO SELECT******************************************************************************//
                ...
                echo json_encode($VendaVideoPatricia);  
